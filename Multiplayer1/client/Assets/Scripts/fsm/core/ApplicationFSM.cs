...
public class ApplicationFSM : MonoBehaviour
{
    ...
    [Tooltip("Drag in one of the child state game object here as a starting state.")] [SerializeField]
    private ApplicationState _startState = null;
    ...
    private float _heartbeatInterval = 0.1f;
    
    private void SendHeartbeat()
    {
        if ((DateTime.UtcNow - _lastHeartbeatSent).TotalSeconds >= _heartbeatInterval)
        {
            channel.SendMessage(new Heartbeat());
